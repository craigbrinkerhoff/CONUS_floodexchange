...
##BIG TO DOS
#(3/21/25): fix tar_combine and figures so that we aren't just combining millions of nhd reaches...
...
gageAnalysis <- tar_map(
  values = tibble( # Setup static branching
    ...
    huc4 = '0108'#c('0101', '0102', '0103', '0104', '0105', '0106', '0107', '0108', '0109', '0110')
  ),
  names='huc4',
  ...
  tar_target(gageQexc, calc_Qexc('deploy', gageRecord, gagePrepped, depthAHG, maxProbDiff),
  ...
  tar_target(reaches_val, collectValReaches(huc4, usgs_maps)), #grab reaches with usgs volume models
  tar_target(for_val, wrangleDepthGrids(huc4, reaches_val)),
  tar_target(gageFlux_val, buildGageFloodFunctions_val(huc4, BHGmodel, for_val)),
...
list(
  ...
  tar_target(gageQexc_val, calc_Qexc('val', gageRecord_val, gagePrepped_val, depthAHG_val, maxProbDiff),
          pattern=map(gageRecord_val),
          iteration='list'),

  ##FIGURES
  tar_target(fig_validationQexc, plotValidationQexc(gageQexc_val)),
  tar_target(valVolFig, makeVolValFig(gageVolume_val_combined))
