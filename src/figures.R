...
makeValFEMA <- function(val_FEMA_combined, gage_combined, BHGdata){
    ...
    scatter_inn <- ggplot(val_FEMA_combined, aes(x=A_femaAEP_km2*1e6, y=A_qFEMA_km2*1e6, color=huc4))+
        ...
        ggtitle('Floodplain + channel inundated area')+
    ...
    scatter_flood <- ggplot(val_FEMA_floodplain, aes(x=Af_femaAEP_km2*1e6, y=Af_qFEMA_valmodel_km2*1e6, color=huc4))+
        ...
        theme(axis.title = element_text(face = 'bold', size=18),
            ...
            legend.position='none',
            plot.title = element_text(size=22)) +
        ggtitle('Floodplain inundated area')+
        annotate("text", x = 1e6, y = 10^1.5, label = bquote(.(nrow(val_FEMA_floodplain))*' rivers'), size=6)+
        annotate("text", x = 1e6, y = 10^1, label = bquote(r^2*': '*.(round(summary(lm_flood)$r.squared,2))), size=6)
    ...
    comboPlot <- patchwork::wrap_plots(A=scatter_inn, B=scatter_flood, design=layout)

    ggsave('cache/validationFEMA.png', comboPlot, width=8, height=15)
